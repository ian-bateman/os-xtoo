...
src_prepare() {
	rm "${WORKDIR}/asterisk-patchset/05-invert-gmime-search-order.patch" \
		|| die "Remove legacy patch"
	default
...
src_install() {
	...
	newinitd "${FILESDIR}"/asterisk.initd8 asterisk
	...
	#
	dodoc "${FILESDIR}/sip_calc_auth"
	dodoc "${FILESDIR}/find_call_sip_trace.sh"
	dodoc "${FILESDIR}/find_call_ids.sh"
	dodoc "${FILESDIR}/call_data.txt"

	# install logrotate snippet; bug #329281
	#
	insinto /etc/logrotate.d
	newins "${FILESDIR}/asterisk.logrotate4" asterisk
}
