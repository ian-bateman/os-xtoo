...
src_prepare() {
	...
	java-pkg_jar-from --into libs.lucene/external lucene-core-3 lucene-core.jar lucene-core-3.5.0.jar
	...
	# jna:4
	sed -i -e 's|APISupport.class, DEFAULT_OPTIONS|APISupport.class|' \
		dlight.nativeexecution/src/org/netbeans/modules/nativeexecution/support/Win32APISupport.java \
		|| die "Failed to sed Win32APISupport for jna:4"

	# Drop activation.jar
	sed -i -e '/external\/activation/d' \
		xml.jaxb.api/nbproject/project.properties \
		|| die "Delete activation.jar"
